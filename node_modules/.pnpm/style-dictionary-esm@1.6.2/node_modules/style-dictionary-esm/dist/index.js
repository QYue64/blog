...
function convertToBase64(filePath) {
  ...
  const body = fs4.readFileSync(filePath, "binary");
...
function resolveTemplate(_path) {
  ...
  return fs4.readFileSync(join(_dir, _path));
}
...
`;
  ...
  "typescript/module-declarations": function({ dictionary, file, options: options2 }) {
    ...
    const _dir = resolve(dirname(fileURLToPath(import.meta.url)), "../templates");
    const designTokenInterface = fs4.readFileSync(resolve(_dir, "../../types/DesignToken.d.ts"), { encoding: "UTF-8" });
...
var actions_default = {
  ...
  "android/copyImages": {
    do(dictionary, config) {
      ...
      dictionary.allTokens.forEach((token) => {
        if (token.attributes.category === "asset" && token.attributes.type === "image") {
          ...
          fs4.cpSync(token.value, `${imagesDir + token.attributes.state}/${name}.png`);
        }
      });
    },
    undo(dictionary, config) {
      ...
      dictionary.allTokens.forEach((token) => {
        if (token.attributes.category === "asset" && token.attributes.type === "image") {
          const name = token.path.slice(2, 4).join("_");
          fs4.rmSync(`${imagesDir + token.attributes.state}/${name}.png`);
        }
      });
    }
  },
  ...
  "copy_assets": {
    do(dictionary, config) {
      ...
      fs4.cpSync("assets", `${config.buildPath}assets`);
    },
    undo(dictionary, config) {
      logger_default().log(`Removing assets directory from ${config.buildPath}assets`);
      fs4.rmSync(`${config.buildPath}assets`);
    }
  }
};
...
function createReferenceRegex(opts = {}) {
  ...
  return options2?.regex || new RegExp(
    `\\${options2.openingChar}([^${options2.closingChar}]+)\\${options2.closingChar}`,
    "g"
  );
}
...
function usesReference(value, referencesOptionsOrRegex = {}) {
  const self = this;
  const regex2 = referencesOptionsOrRegex instanceof RegExp ? referencesOptionsOrRegex : createReferenceRegex_default({
...
function resolveObject(object, opts) {
  ...
  regex = createReferenceRegex_default(options?.referencesOptions || {});
...
function exportPlatform(platform) {
  ...
  while (typeof finished === "undefined") {
    ...
    exportableResult = resolveObject_default(transformed, { ignorePaths: ignorePathsToResolve, referencesOptions: platformConfig.referencesOptions });
    ...
    if (newDeferredPropCount === 0) {
      ...
      resolveObject_default(transformed, { referencesOptions: platformConfig.referencesOptions });
      finished = true;
    } else {
...
function createFormatArgs({ dictionary, platform, file = {} }) {
  const { allProperties, properties, allTokens, tokens, usesReference: usesReference2, getReferences: getReferences2, referencesOptions } = dictionary;
...
function buildFile(file = {}, platform = {}, dictionary = {}) {
  ...
  if (fullDestination && platform.write !== false) {
    const dirname5 = path4.dirname(fullDestination);
    if (!fs4.existsSync(dirname5))
      fs4.mkdirSync(dirname5, { recursive: true });
  }
  ...
  if (filteredProperties.hasOwnProperty("properties") && Object.keys(filteredProperties.properties).length === 0 && filteredProperties.properties.constructor === Object) {
    ...
    !silent && logger_default().log(chalk.red(warnNoFile));
    return null;
  }
  ...
  if (!silent) {
    if ((nested || propertyNamesCollisionCount === 0) && filteredReferencesCount === 0) {
      logger_default().log(chalk.bold.green(`\u2714\uFE0E ${fullDestination}`));
    } else {
...
${help}`;
        logger_default().log(chalk.red.bold(warn));
      }
...
function getReferences(value, references = []) {
  const self = this;
  const regex2 = createReferenceRegex_default(self.referencesOptions || {});
...
${platform}`);
  ...
  const dictionary = createDictionary_default({ properties }, platformConfig?.references || {});
...
function cleanFile(file = {}, platform = {}) {
  ...
  if (!fs4.existsSync(destination)) {
    !silent && logger_default().log(`${chalk.bold.red("!")} ${destination}, does not exist`);
    return;
  }
  fs4.unlinkSync(destination);
  !silent && logger_default().log(`${chalk.bold.red("-")} ${destination}`);
}
...
function cleanDir(file = {}, platform = {}) {
  ...
  if (platform.buildPath)
    destination = platform.buildPath + destination;
  let dirname5 = path4.dirname(destination);
  while (dirname5) {
    if (fs4.existsSync(dirname5)) {
      if (fs4.readdirSync(dirname5).length === 0) {
        !silent && logger_default().log(`${chalk.bold.red("-")} ${dirname5}`);
        fs4.rmdirSync(dirname5);
      } else {
        break;
      }
    }
...
function resolveFileContent_default(resolvedPath) {
  ...
  if (resolvedPath.match(/.json5$/)) {
    const source = fs4.readFileSync(resolvedPath, { encoding: "UTF-8" });
    ...
    try {
      parsers.forEach(({ pattern, parse }) => {
        if (resolvedPath.match(pattern)) {
          file_content = parse({
            contents: fs4.readFileSync(resolvedPath, { encoding: "UTF-8" }),
